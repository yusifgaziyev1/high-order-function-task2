...
let result = users.filter((findCity) => findCity.address.city == "New York");
console.log(result.length);

// NOT 1: Burada şəhərin adı manuel olaraq qeyd edilib ancaq bu da işləyir. 

// 2)

const getFindCityPerson = (city) => {
    let result = users.filter((findCity) => findCity.address.city == city);
    console.log(result.length);
    
}

getFindCityPerson("Phoenix");

// NOT 2: Burada isə, getFindCityPerson funksiyasını çağıranda mötərizənin içinə hansı şəhərin adını yazsaq o şəhərdə yaşayan insanların sayını göstərəcək.
...
let result2 = users.filter((findRole) => findRole.role == "admin");
console.log(result2.length);

// NOT 1: Burada rolenin adı manuel olaraq qeyd edilib ancaq bu da işləyir. 

// 2)

const getFindRole = (role) => {
    let result = users.filter((findRole) => findRole.role == role);
    console.log(result);
    console.log(result.length);
}

getFindRole("admin");
getFindRole("moderator");
getFindRole("user");
getFindRole("super admin"); // super admin olmadığı üçün boş array verdi

// NOT 2: Burada isə, getFindRole funksiyasını çağıranda mötərizənin içinə hansı role adını yazsaq o roleye aid insanların siyahısını göstərəcək.



// 3) "department": "Accounting" olanlari secib array sekilinde ekrana cixaran func yazin

let result3 = users.filter((findAccounting) => findAccounting.company.department == "Accounting");
console.log(result3);

// 4) emailde gmail.com ile bitenleri secib bir arraya yigan func yazin

let result4 = users.filter((findEmail) => findEmail.email.includes("gmail.com") == true);
console.log(result4);

// QEYD: Array yenilenib isitifade ederken burdaki arrayi kopyalayin
